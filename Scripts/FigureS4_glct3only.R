...
cor_plot <- pr_roi_ld %>%
  ...
  geom_rect(aes(xmin = ifelse(strand == "+", txstart/1e6, txend/1e6),
                ...
                ymin = 1.05,
                ymax = 0), size = 1, data = gene_df %>% dplyr::filter(gene_id == "WBGene00011781"), alpha = 0.25) +
  geom_text(aes(x = plot_pos/1e6,
                label = gene_name,
                y = 1.08), fontface="italic", data = gene_df %>% dplyr::filter(gene_id == "WBGene00011781")) +
  ...
  geom_segment(aes(x = ifelse(strand == "+", txstart/1e6, txend/1e6),
                   ...
                   y = 1.05,
                   yend = 1.05),
               arrow = arrow(length = unit(5, "points")), size = 1, data = gene_df %>% dplyr::filter(gene_id == "WBGene00011781")) +
  labs(x = "Genomic Position (Mb)",
       y = expression(italic(r)^2)) 


cowplot::plot_grid(gene_plots[[4]]+theme(axis.title.y = element_blank()), 
                   cor_plot,
                   ncol = 1, align = "hv",
                   label_size = 14, labels = c("A","B"), rel_heights = c(1,.8))


ggsave(filename = "Plots/SVG_PLOTS/FigureS4.png", height = 10, width = 7.5, units = "in", dpi=300)
ggsave(filename = "Plots/SVG_PLOTS/FigureS4.pdf", height = 10, width = 7.5, units = "in")
ggsave(filename = "Plots/SVG_PLOTS/FigureS4.svg", height = 10, width = 7.5, units = "in")
